...
export default function AddNews({ setShowNavbar }: AddNewsProps) {
  ...
  const [values, setValues] = useState<NewsState>(initialState);
  const [progress, setProgress] = useState<number>(0);
  const [file, setFiles] = useState<File[]>([]);
  const [videoUrl, setVideoUrl] = useState<string>("");
  const [imageUrl, setImageUrl] = useState<string>("");
  ...
  useEffect(() => {
    if (imageUrl !== "" || videoUrl !== "") {
      setValues((prevValues) => ({
        ...
        image: imageUrl,
        video: videoUrl,
        user_id: userLoginState.currentUser.id,
      }));
    }
  }, [imageUrl, videoUrl]);
    ...
    return (
      <>
        ...
        <Box sx={style}>
          <Typography id="modal-modal-title" variant="h6" component="h2">
            <Typography gutterBottom>
              Incorrect Username or Password! Or You haven't got approval for job!  
              CLICK ON LOGOUT BUTTON TO MOVE TO ANOTHER PAGE!
            </Typography>
          </Typography>
        </Box>
